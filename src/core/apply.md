...
Codex computer-use only. Input: allowlisted companies.
...
5. Log `Company | Role | Applied | Salary | Id` via Google Sheets MCP. `Id` is the Fantastic.jobs integer `id`. Skip if that `id` is already on the sheet. Do not submit.
6. Wait for user approval to submit. Start the next allowlisted application in the meantime.
